  ...
  return (
    ...
    >
      <motion.div
        ...
        className="flex h-full w-full flex-col bg-white md:max-w-[1140px] md:overflow-hidden md:rounded-[24px] md:shadow-2xl"
        onClick={isMobile ? undefined : (clickEvent) => clickEvent.stopPropagation()}
      >
        <motion.div
          initial={false}
      ...
      {!isMobile && lightboxOpen ? (
        <Lightbox images={images} title={event.title} onClose={() => setLightboxOpen(false)} />
      ) : null}
    </motion.div>
  );
}
