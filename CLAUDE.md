...
src/
  components/
    ...
    map/
      MapView.jsx
    chat/
      ChatPanel.jsx        # Web: 400px slide-in panel, backdrop overlay, FAB-toggled
...
- **Gallery / Lightbox:** built — `src/components/gallery/Lightbox.jsx` (web fullscreen overlay, prev/next, counter, vertical filmstrip, ←/→/Escape) and `src/components/gallery/GalleryScreen.jsx` (`/gallery/:id` route, black fullscreen, X button + centered counter, swipeable image, horizontal filmstrip) both exist and are wired — `DetailPage.jsx`'s "View all photos" opens `Lightbox` directly at index 0; clicking any individual mosaic image (`ImageMosaic.jsx`'s `onImageClick`) opens it at that image's index instead (`Lightbox` now takes an `initialIndex` prop). `GalleryScreen`'s header was a back arrow — now an X (`handleClose`): web (`window.innerWidth >= 768`) still goes to `/festival/:id`.
  - **Mobile close gap fixed:** previously `navigate(-1)` landed back on `/` without restoring `DetailModal`'s open state, since opening the modal never pushes a history entry (it's pure React state owned by `HomePage`) — `navigate(-1)` from `/gallery/:id` pops straight past it to whatever entry preceded the modal-open, and `HomePage` remounts with `selectedEvent` reset to `null`. Root cause meant a real fix couldn't touch `App.jsx`/`HomePage` (state isn't in the URL) without lifting that state — instead, fixed entirely within `GalleryScreen.jsx`: it now imports `DetailModal` directly and renders it itself (`{modalEvent ? <DetailModal event={modalEvent} events={events} onClose={() => navigate(-1)} onSelectEvent={setModalEvent} /> : null}`) when the mobile close button is tapped, rather than navigating away at all — the URL stays on `/gallery/:id`, `DetailModal`'s own `fixed inset-0 z-[60]` overlay just covers the gallery underneath. `DetailModal` is otherwise completely unmodified; `onSelectEvent={setModalEvent}` reuses the exact same "swap which event the modal shows, no navigation" pattern `HomePage` already uses for its own modal, so tapping a related festival inside this reopened modal works identically. *Closing* this reopened modal calls the real `navigate(-1)`, which correctly pops back to `/` since the gallery's forward-push is still the only intervening history entry. Verified live: tap card → modal opens (`/`) → "View all photos" → gallery (`/gallery/diwali`) → close gallery → modal reopens showing the same festival, URL still `/gallery/diwali` → close modal → back to the card grid at `/`.
...
- `src/hooks/usePexels.js` — throttles concurrent Pexels requests (max 15 in flight) to avoid 429s; image objects carry `large2x`/`original` sizes. **Fixed: card grid was firing one Pexels request per card on page load (155 total).** Two changes, scoped to the home/calendar card-grid path only (`FestivalCard.jsx` + this hook) — map, detail pages, gallery, and FAB chat untouched:
  - **Caching:** the old `src/cache/pexels-cache.json` file-read/PUT-write cache was already a documented no-op (nothing ever persisted the PUT) — removed entirely and replaced with a real in-memory `Map` (module-level, so it survives client-side navigation between tabs/routes within the SPA session, but resets on a hard page reload — **deliberately not `localStorage`**, since CLAUDE.md's "No localStorage" rule was flagged to the user and they chose the in-memory approach over amending that rule). `usePexels(imageQuery, cacheId)` now takes an optional second arg; when provided (as `FestivalCard.jsx` does, passing `event.id`) it's used as the cache key instead of the `imageQuery` string, so each festival entry caches independently of any other entry that happens to share a query term. All other callers (`DetailPage`, `DetailModal`, `GalleryScreen`, `MapView`, `CalendarView`) still call with just `imageQuery` and are unaffected besides now getting a cache that actually works.
  - **Lazy loading:** `FestivalCard.jsx` now uses an `IntersectionObserver` (`rootMargin: '200px'`, one-shot — disconnects after first intersection) to defer `fetchImages()` until the card is about to scroll into view; before that, it renders a `animate-pulse` grey skeleton in place of the image. Verified live: initial page load fires ~11 Pexels requests (only the cards in/near the first viewport) instead of 155; scrolling down triggers more, scrolling back up over already-seen cards fires zero additional requests (cache hit).
  - **`CalendarView.jsx` now fixed too**, same treatment: `CalendarEventRow` passes `event.id` as `cacheId` to `usePexels`, gates its `fetchImages()` call behind a one-shot `IntersectionObserver` (`rootMargin: '200px'`) on the row's root button, and shows an `animate-pulse` skeleton in the 48px thumbnail slot until the row is visible. Verified live: opening the Calendar tab now fires ~9 Pexels requests (just the visible rows) instead of bursting for all 155; scrolling down/back up produces no duplicate fetches for already-seen rows.
...
- **Centering bug found, then the whole page-compression mechanism removed:** `<main>` originally had both `mx-auto` (class) and an inline `style={{ marginRight: chatMode === 'panel' ? 400 : 0 }}` to shrink the page when the chat panel opened. An inline `style` always wins over a class for the same CSS property, so the inline `margin-right` (even when `0`) silently overrode the class's `margin-right: auto`, breaking the centering — `<main>` stayed clamped to exactly 1140px (no overflow) but sat pinned flush to the right edge with all the slack collapsed into `margin-left` instead of split evenly. First fix moved the `marginRight` to the outer wrapper div so `<main>` could re-center within the remaining space — but the correct fix turned out to be removing page compression entirely: **the chat panel must be a pure `position: fixed` overlay that never affects the main content's layout, width, or position.** `App.jsx`'s outer wrapper no longer carries any `marginRight`/transition at all; `<main>` is always `mx-auto max-w-[1140px]` with zero inline margin override, in every chat state. Verified: `<main>`'s `getBoundingClientRect()` is byte-identical before and after opening the panel at a 1920px viewport (`left: 390, right: 1530, width: 1140` in both states). `ChatPanel.jsx`'s panel (`fixed inset-y-0 right-0 z-50`) and backdrop (`fixed inset-0 z-40 bg-black/30`, default `pointer-events: auto`, `onClick={onClose}`) were already correctly `position: fixed` and needed no changes — the shift was coming entirely from the ancestor margin, not the panel itself.
- **`DetailPage.jsx`'s sticky collapsed-header bar now shares the same 1140px constraint** as the content below it — previously the bar was `fixed inset-x-0` with only `px-8` (no `max-w`/`mx-auto`), so its title/X button sat flush at the far edges of the viewport on wide screens while the mosaic/accordions/related content below stayed clamped to 1140px, misaligning the two on anything wider than ~1200px. Fixed by wrapping the bar's inner content in its own `mx-auto max-w-[1140px] px-8` div; verified all three of `DetailPage.jsx`'s constrained containers (sticky bar, hero/mosaic, content) report identical `left`/`width` at a 1920px viewport.
...
- **FAB chat:** complete — **rebuilt to be fully local, no Anthropic API call at all** (this replaces the original Claude-API-backed version: removed `fetch` to `https://api.anthropic.com/v1/messages`, the system-prompt builder, and the compact-database `useMemo` entirely). `useChat.js` now does keyword-based local search over the full `india-cultural-calendar.json` import: `extractKeywords(query)` lowercases the input, tokenizes on `[a-z0-9]+`, and strips a `STOP_WORDS` set (filler words like "tell", "me", "about", "which", "festival(s)", "celebrated", "significance", question words) so natural-language questions reduce to their meaningful terms (e.g. "What is the significance of Holi?" → `["holi"]`). `matchesKeyword(entry, keyword)` does case-insensitive substring matching against `title`/`keywords[]`/`state[]`/`whyCelebrated`/`religion`, plus exact match against `month`; `searchFestivals` returns entries matching ANY extracted keyword (OR), capped at 5. `generateLocalResponse` formats a single match as `**Title** (Month)\n\nwhyCelebrated\n\nhowCelebrated`, multiple matches as a bulleted list with bolded titles and an 80-char `whyCelebrated` snippet, and a fixed "couldn't find anything" string when zero match. `sendMessage` is now synchronous (no `async`/`try-catch` — local array filtering can't fail) and instant, same philosophy as the AI Summary box below. Verified live: zero network requests to `anthropic.com` fire (confirmed via a request listener), and all 4 starter pills — including full sentences like "Tell me about Onam" — now resolve to real results after the keyword-extraction step (without it, all 4 returned "couldn't find anything," since the original literal/substring design only matched short keyword-style input, not full questions).
  - **Exact-title-match short-circuit added** to fix a real ranking bug: "tell me about holi" extracted to `["holi"]` correctly, but `matchesKeyword`'s plain substring OR-search buried the actual Holi entry behind false positives (e.g. `"holi"` matching inside `"holistic"` in International Day of Yoga's `whyCelebrated`), returning a 5-item list with Holi not even first. `searchFestivals` now calls `findExactTitleMatch(keywords, data)` first — it normalizes each title (`normalizeTitle`: lowercase, strip to `[a-z0-9]+` tokens, rejoin with spaces) and compares against the extracted keywords joined the same way; if a festival's normalized title exactly equals the joined keywords, that single entry is returned immediately (skipping the substring search entirely), which both ranks it first and forces the single-festival response format instead of a list. Verified: "tell me about holi" and "What is the significance of Holi?" both now resolve to `["Holi"]` only. This only fires for single-concept queries whose stripped keywords match a title exactly (mostly single-word titles — "Holi", "Onam", "Vishu"); multi-word titles like "Diwali (Deepavali)" still fall through to the existing substring OR-search (unaffected, no regression — "diwali" still correctly returns Diwali first among 3 results).
  ...
  - FAB (`src/components/layout/FAB.jsx`) still takes `isOpen`/`onClick`, swaps `SparklesIcon` ↔ `Cancel01Icon`, `z-[60]` so it stays clickable above the chat panel/overlay (both `z-50`). `App.jsx`'s `HomePage` still picks `'panel'` vs `'overlay'` via a one-time `window.innerWidth < 768` check at the moment the FAB is clicked, stored in `chatMode` state.
  - **FAB now fully hides whenever chat is open, in either mode** (`{!chatMode ? <FAB .../> : null}`, was `{chatMode !== 'overlay' ? ... : null}`) — previously the FAB stayed mounted (as the X icon) while the web panel was open, fixed bottom-right at `z-[60]` above the panel, which looked like a second, redundant close button floating below the chat input. The panel's own top-right X (in its header) is the only close affordance now while it's open; FAB reappears (sparkle icon) once chat is fully closed. `isOpen` prop on the `<FAB>` call site was dropped since it's now always closed when FAB renders.
  - **Header renamed "Ask about festivals" → "Ask anything"** in both `ChatPanel.jsx` (web) and `ChatOverlay.jsx` (mobile) — these were the two literal headings asked for. `FAB.jsx`'s own `aria-label="Ask about festivals"` (the trigger button's accessibility label, not a heading) was deliberately left as-is — different element, not named in the request.
  - **Web panel widened to 400px (from 320px) and given a click-to-close backdrop**, matching `FiltersPanel.jsx`'s sheet pattern: a `bg-black/30` full-screen overlay at `z-40` sits behind the panel (`z-50`) and calls `onClose` on click. Implemented with Framer Motion (`x: 400 → 0`, 0.3s) rather than raw Tailwind `transition-transform`/`translate-x-*` classes, to stay consistent with the rest of the codebase's animation approach (`FiltersPanel` itself uses Framer Motion `x`/`y` animate, not CSS transition classes, despite both achieving the same slide-in visual). Mobile `ChatOverlay.jsx` is unchanged — this fix was scoped to web only.
